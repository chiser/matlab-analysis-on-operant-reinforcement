...
%         exclude(ee,1:size(flyTracks.rightTurns,2))=flyTracks.numTurns<14;
        ...
        arm=nan(max(flyTracks.numTurns),length(flyTracks.currentarm(1,:)));
        leftArm=[];
        middleArm=[];
        rightArm=[];
        left_dwell_time=[];
        ...
        StampsTurns=[];
        order=[];
        for i=1:length(flyTracks.currentarm(1,:))

        leftArm{i}=find(flyTracks.currentarm(:,i)==1);
        middleArm{i}=find(flyTracks.currentarm(:,i)==2);
        rightArm{i}=find(flyTracks.currentarm(:,i)==3);
